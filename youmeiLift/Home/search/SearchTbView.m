...
@interface SearchTbView ()<TagViewDelegate, UISearchBarDelegate>
...
- (void)setUpSearchTBViewWithImgStr:(NSString *)ImgStr videoStr:(NSString *)videoStr{
    ...
    FLAnimatedImageView *jsImgView = [[FLAnimatedImageView alloc]init];
//    jsImgView.contentMode = UIViewContentModeScaleAspectFit;
//    jsImgView.backgroundColor = [UIColor whiteColor];
    [_appScrollView addSubview:jsImgView];
    
    
                   self.imgUrlStr = [NSString stringWithFormat:@"%@", ImgStr];
                   
                  
                   self.videoUrlStr = [NSString stringWithFormat:@"%@", videoStr];
                   
                   if (self.imgUrlStr.length > 6) {
                       
                       [jsImgView mas_makeConstraints:^(MASConstraintMaker *make) {
                              
                              make.top.equalTo(_appScrollView.mas_top).offset(10);
                              make.left.equalTo(_appScrollView.mas_left).offset(10);
                          
                              make.size.mas_equalTo(CGSizeMake(kScreenWidth - 20, 100*BiLi));
                              
                          }];
                       
                       UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(clickJsImgView:)];
                       [jsImgView addGestureRecognizer:tap];
                       jsImgView.userInteractionEnabled = YES;
                       
                         NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@", self.imgUrlStr]];
                       [jsImgView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"home_bannerEmpty.png"]];
                       
                   }else{
                       
                       [jsImgView mas_makeConstraints:^(MASConstraintMaker *make) {
                              
                            make.top.equalTo(_appScrollView.mas_top).offset(0);
                            make.left.equalTo(_appScrollView.mas_left).offset(10);
                                                       
                            make.size.mas_equalTo(CGSizeMake(kScreenWidth - 20, 0));
    ...
    [hotSearchLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(jsImgView.mas_bottom).offset(24);
...
- (void)clickJsImgView:(UIGestureRecognizer *)sender{
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(chlickVideoView)]) {
           [self.delegate chlickVideoView];
       }
...
- (void)setUpSearchTBViewWithArr:(NSMutableArray *)hotSearchArr{
    ...
    if (self.imgUrlStr.length > 6) {
        self.tagView = [[TagView alloc]initWithFrame:CGRectMake(5, 45+110, kScreenWidth - 10, 0)];
...
-(void)handleSelectTag:(NSString *)keyWord{
   ...
   // self.passSearchName(keyWord);
    self.passTBSearchName(keyWord, @"11");
}

- (void)handleSelectTagDict:(NSDictionary *)dict{
    NSString *ttStr = [NSString stringWithFormat:@"%@", dict[@"title"]];
       if (ttStr) {
           self.passTBSearchName(ttStr, @"00");
       }
    self.passTBSearchDict(dict);
}
...
- (void)clickSearchButton:(UIButton *)sender{
    ...
    if (trimedString.length == 0) {
        ...
        self.passTBSearchName(trimedString, @"11");
    }
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    self.passTBSearchNameOnTime(searchBar.text);
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(chlickSearchTbView)]) {
        [self.delegate chlickSearchTbView];
    }
...
- (void)setUpTBSearchHistoryViewWithArr:(NSMutableArray *)historyArr{
    ...
    [_historyView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        DMLog(@"tagView.frame.size.height == %f", tagView.frame.size.height);
    
        
                  if (self.imgUrlStr.length > 6) {
                       height   =110+ 105+self.tagView.frame.size.height +60 +  tagView.frame.size.height +20;
                  }else{
                  ...
                  if (height > kScreenHeight-41-(iPhoneX?88:64)) {
                      ...
                      [_historyView mas_updateConstraints:^(MASConstraintMaker *make) {
                          make.size.mas_equalTo(CGSizeMake(kScreenWidth, kScreenHeight-41-(iPhoneX?88:64)));
                      }];
...
- (void)refreshTBSearchHistoryViewWithArr:(NSMutableArray *)historyArr{
    ...
    if ([self.historyType isEqualToString:@"nooo"]) {
        ...
        DMLog(@"tbtb刷新 historyArr== %@", historyArr);
