...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.styleFlood == YES) {
        height = self.frame.size.height;//为了字体在中间显示
    }
    ...
    if (self.isTriangle) {
     ...
     if (self.styleFlood == YES) {
         //为了字体在中间显示
         UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(startX, lineWidth / 2.0, width, height - lineWidth) cornerRadius:self.styleFlood?12:2];
    ...
    if (self.hasBorder) {
        ...
        CGFloat endX = CGRectGetMaxX([self.itemFrames.lastObject CGRectValue]);
         ...
         #pragma mark---- 改过 self.cornerRadius 改为 2
        
        UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(startX, lineWidth / 2.0, (endX - startX), height - lineWidth) cornerRadius:self.styleFlood?12:2];
