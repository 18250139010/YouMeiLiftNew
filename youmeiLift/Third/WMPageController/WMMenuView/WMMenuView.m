...
- (void)resetFramesFromIndex:(NSInteger)index {
    ...
    if (self.style == WMMenuViewStyleLine || self.style == WMMenuViewStyleTriangle) {
        ...
        frame = CGRectMake(0, (self.frame.size.height - self.progressHeight) / 2+0.5, self.scrollView.contentSize.width, self.progressHeight);//+0.5 使字体剧中
...
- (NSArray *)convertProgressWidthsToFrames {
    ...
    for (int i = 0; i < count; i++) {
        ...
        //NSLog(@"23334**== -- 233==  %@", self.progressWidths[i]);
        ...
        CGFloat x = itemFrame.origin.x + (itemFrame.size.width - progressWidth) / 2;
         //  #pragma mark ---  itemFrame.origin.y  -->  itemFrame.origin.y - 15
...
//    NSLog(@"caca233 == %@",NSStringFromCGRect(frame));
...
- (void)addScrollView {
    ...
    if ([self.styleWPH isEqualToString:@"wph"]) {
         scrollView.backgroundColor = kUIColorFromRGB(0xE9E8E8);
        scrollView.layer.masksToBounds = YES;
        scrollView.layer.cornerRadius = 16.0f;
    }
    else{
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        if (i == 0) {
         ...
         #pragma mark -------
        // ***替换***
        if ([[self.dataSource menuView:self titleAtIndex:i] isEqualToString:@"今日上新"]) {
            item.text = @"";
              CGRect frame = [self.frames[0] CGRectValue];
            NSLog(@"今日上新 frame = %@",NSStringFromCGRect(frame));
            UIImageView *oneNewimgView = [[UIImageView alloc]initWithFrame:CGRectMake(7.5, 8.5, frame.size.width - 15, 24)];
            oneNewimgView.image = [UIImage imageNamed:@"home_mrtj.png"];
           // oneNewimgView.backgroundColor = [UIColor yellowColor];
            [self.scrollView addSubview:oneNewimgView];
        }
...
- (void)addProgressViewWithFrame:(CGRect)frame isTriangle:(BOOL)isTriangle hasBorder:(BOOL)hasBorder hollow:(BOOL)isHollow cornerRadius:(CGFloat)cornerRadius {
    ...
    NSLog(@"2345**-- %@",NSStringFromCGRect(frame));
    
    WMProgressView *pView = [[WMProgressView alloc] initWithFrame:frame];
    
    pView.itemFrames = [self convertProgressWidthsToFrames];
  //  pView.color = ColorMaker(255, 151, 54, 255).CGColor;
    
      pView.styleFlood = NO;
         
    
    if (self.style == WMMenuViewStyleFlood) {
       if ([self.styleWPH isEqualToString:@"wph"]) {
           self.lineColor = kUIColorFromRGB(0xFFFFFF);
            pView.styleFlood = YES;
       }else{
           self.lineColor = kUIColorFromRGB(0xF84940);
            pView.styleFlood = YES;
       }
    }
