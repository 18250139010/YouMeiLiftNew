...
//    if (self.menuViewStyle == 5) {
//          _titleColorNormal   = [UIColor blueColor];
...
- (void)wm_addMenuView {
    ...
    //self.menuView =[[WMMenuView alloc] initWithFrame:CGRectZero];
    ...
    if (self.titleFontName) {
       menuView.fontName = self.titleFontName;//注释掉，选中的时候用字体，常规的不变
    }
    if (self.styleWPH) {
        menuView.styleWPH = self.styleWPH;
    }
...
//            WMMenuView *view1 = (WMMenuView *)view;
...
- (CGFloat)menuView:(WMMenuView *)menu widthForItemAtIndex:(NSInteger)index {
    if (self.automaticallyCalculatesItemWidths) {
        ...
        NSLog(@"自适应自适应");
        return ([self wm_calculateItemWithAtIndex:index] +20);
    }
...
//    if ([[self titleAtIndex:index] isEqualToString:@"今日上新"]) {
//         return @"11";
//    }else{
